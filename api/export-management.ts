...
export default async function handler(req: IncomingMessage & { body?: any; method?: string }, res: ServerResponse) {
  ...
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="${channel}_${period}_Management_Report.docx"`);
    return res.end(buffer);
  } catch (error: any) {
    console.error('Management export failed:', error);
    res.statusCode = 500;
    return res.end(`Management export failed: ${error?.message || 'Unknown error'}`);
  }
}
